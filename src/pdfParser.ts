...
// Initialize the PDF.js library with a fallback dynamic CDN worker setup
...
export async function parsePDFFile(file: File): Promise<{ pageCount: number; pages: ExtractedPage[] }> {
  try {
    ...
    // Parse each page up to 10 pages for preview safety
    ...
    for (let pageNum = 1; pageNum <= maxPagesToParse; pageNum++) {
      ...
      if (items.length === 0) {
        // Fallback for scanned/empty pages
        extractedPages.push({
          pageNumber: pageNum,
          width: pageWidth,
          height: pageHeight,
          blocks: [
            {
              id: `p-${pageNum}-empty`,
              type: "paragraph",
              originalText: `[Page ${pageNum}: Scanned document or empty content detected]`,
              x: 15,
              y: 45,
              w: 70,
              h: 10,
